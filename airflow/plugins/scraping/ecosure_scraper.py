...
def scrape_faqs(url):
    ...
    question_elements = soup.find_all(class_='q')
    for question_element in question_elements:
        question_text = question_element.get_text(strip=True)
        questions.append(question_text)
        
        answer_element = question_element.find_next_sibling(class_='a')
        if answer_element:
